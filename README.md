...
Media Pulse 是一套面向品牌机型与竞品机型的社媒与媒体反馈分析流水线，当前已经不只是 `camera-only` 抓取，而是覆盖了：

1. 多源内容采集与去重
2. camera 相关过滤与 AI 结构化标签
3. 视频转写 / 评论区挖掘 / 竞品视频归类
4. 工作台同步、Dashboard 任务状态、日报与 HTML 周报邮件
...
- Dashboard：黑白高级感视觉，统一邮件报告风格，支持总览 / 任务状态 / 信号洞察 / 竞品视频 / 反馈明细
- 任务状态页：按当前时间范围展示采集、竞品检索、视频分析、工作台同步、报告生成的最新运行状态
...
- `media_pulse/`：对外统一入口包，推荐使用 `python -m media_pulse.cli ...`
...
- 对外建议统一使用 `media_pulse` 作为项目名和命令入口。
- 当前仓库内部仍保留 `nt_cam_pulse/` 模块路径，以兼容已有脚本和历史数据。
...
```bash
# 1) 跑一次全链路抓取
python -m media_pulse.cli --config config.yaml run

# 2) 处理待分析视频
python -m media_pulse.cli --config config.yaml video-process --limit 20

# 3) 同步到工作台
python -m media_pulse.cli --config config.yaml sync-lark --limit 200

# 4) 生成或发送周报
python -m media_pulse.cli --config config.yaml weekly-email --scope camera --send
```
...
```bash
python -m media_pulse.cli --config config.yaml run --dry-run
```

### 检索竞品视频并自动归类

```bash
python -m media_pulse.cli --config config.yaml competitor-video \
  --target "Device A Pro" \
...
```bash
python -m media_pulse.cli --config config.yaml weekly-email \
  --start-date 2026-03-19 \
  --end-date 2026-04-16 \
  --scope camera \
  --top-limit 12 \
  --output ./reports/media-pulse-summary-2026-03-19-to-2026-04-16.html
```
...
```bash
python -m media_pulse.cli --config config.yaml report --date 2026-03-31
```

### 只同步工作台

```bash
python -m media_pulse.cli --config config.yaml sync-lark --date 2026-03-31
```
...
```bash
python -m media_pulse.cli --config config.yaml prepare-lark-dashboard
```
...
```bash
python -m media_pulse.cli --config config.yaml mark-lark-dirty
python -m media_pulse.cli --config config.yaml sync-lark --limit 500 --force-all-updates
```
...
```bash
python -m media_pulse.cli --config config.yaml send-email-summary --date 2026-04-07
```
...
```bash
python -m media_pulse.cli --config config.yaml email-check
```

发送一封测试邮件：

```bash
python -m media_pulse.cli --config config.yaml send-test-email
```

如果要把本地 HTML 预览文件作为测试邮件正文发出去：

```bash
python -m media_pulse.cli --config config.yaml send-test-email --html-file ./reports/weekly-media-email-2026-03-27-to-2026-04-02.html
```
...
```bash
python -m media_pulse.cli --config config.yaml weekly-email --end-date 2026-04-02
```
...
```bash
python -m media_pulse.cli --config config.yaml sync-lark-loop --interval 60 --limit 200
```
...
```bash
python -m media_pulse.cli --config config.yaml backfill --date 2026-03-31 --limit 500
```
...
```bash
python -m media_pulse.cli --config config.yaml retag --limit 5
```

全量重分析并强制回写工作台（包含已同步旧记录）：

```bash
python -m media_pulse.cli --config config.yaml retag --all --sync-lark --sync-batch-limit 200
```

### 导出待补视频转写任务（给 videosummary 用）

```bash
python -m media_pulse.cli --config config.yaml video-tasks --date 2026-03-31 --output-dir ./reports
```

### 手动执行视频分析（调用 videosummary）

```bash
python -m media_pulse.cli --config config.yaml video-process --date 2026-03-31 --limit 5
```

单条处理（用于详情页调试）：

```bash
python -m media_pulse.cli --config config.yaml video-process --id 123
```
...
```bash
python -m media_pulse.cli --config config.yaml ingest-video --url 'https://www.youtube.com/watch?v=D4QyStJWgCc'
```

批量导入（文本文件每行一个链接，支持 `#` 注释）：

```bash
python -m media_pulse.cli --config config.yaml ingest-video --file ./reports/video-links.txt
```

导入后立即触发视频分析：

```bash
python -m media_pulse.cli --config config.yaml ingest-video --file ./reports/video-links.txt --analyze --limit 5
```

最简批处理（默认读取 `./reports/video-links.txt`）：

```bash
python -m media_pulse.cli --config config.yaml ingest-video --analyze --limit 5
```

### 启动每日调度（默认每天 09:00 Asia/Shanghai）

```bash
python -m media_pulse.cli --config config.yaml schedule
```
...
```bash
python -m media_pulse.cli --config config.yaml backend --host 127.0.0.1 --port 8788
```
...
```bash
python -m media_pulse.cli --config config.yaml frontend --host 127.0.0.1 --port 8787 --api-base-url http://127.0.0.1:8788
```
...
```bash
python -m media_pulse.cli --config config.yaml dashboard --host 127.0.0.1 --port 8787
```

说明：`dashboard` 会同时提供 UI 和 `/api/*`，用于单机快速验证；当前界面拆成总览、任务状态、信号洞察、竞品视频、反馈明细几个 tab，其中任务状态页会按当前时间范围显示采集、竞品检索、视频分析、工作台同步、报告生成的处理状态。生产建议前后端分离部署。

## 报告输出

日报默认生成到：

- `reports/media-pulse-YYYY-MM-DD.md`
