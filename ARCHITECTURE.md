...
- `GET /api/status?date=YYYY-MM-DD`：本地 AI 可用性 + 视频处理状态
...
- 分离部署（推荐）
  - Backend：`python -m nt_cam_pulse.cli --config config.yaml backend --port 8788`
  - Frontend：`python -m nt_cam_pulse.cli --config config.yaml frontend --port 8787 --api-base-url http://127.0.0.1:8788`
- 一体模式（兼容）
  - `python -m nt_cam_pulse.cli --config config.yaml dashboard --port 8787`
